...
public class JDialog_AddCCAccount extends javax.swing.JDialog {
	...
	public JDialog_AddCCAccount(CardFrm parent, CCard cCard) {
		super(parent);
		this.cCard = cCard;
		parentframe = parent;
	...
	void JButtonOK_actionPerformed(java.awt.event.ActionEvent event) {
		...
		ClientData clientData = getFilledCCardData();
		cCard.addAccount(clientData);
		dispose();
	}

	private ClientData getFilledCCardData() {
		ClientData cCardData = new ClientData();
		cCardData.setAccNum(parentframe.ccnumber);
		cCardData.setName(parentframe.clientName);
		cCardData.setCity(parentframe.city);
		cCardData.setZip(parentframe.zip);
		cCardData.setState(parentframe.state);
		cCardData.setStreet(parentframe.street);
		cCardData.setEmail(parentframe.eMail);
		// to be implemented organization from GUI
		cCardData.setCustType(CustomerType.PERSON);

		if (JRadioButton_Gold.isSelected())
			cCardData.setAccountType(CCardType.GOLD.toString());
		else if (JRadioButton_Silver.isSelected())
			cCardData.setAccountType(CCardType.SILVER.toString());
		else if (JRadioButton_Bronze.isSelected())
			cCardData.setAccountType(CCardType.BRONZE.toString());

		cCardData.setExpDate(parentframe.expdate);
		return cCardData;
	}
